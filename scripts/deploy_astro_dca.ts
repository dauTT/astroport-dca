...
const DCA_MODULE__BINARY_PATH = 'astroport_artifacts/astroport_dca_module.wasm'
...
async function main() {
    ...
    network.DcaCodeId = await uploadContract(terra, wallet, DCA_MODULE__BINARY_PATH)
    ...
    if (!network.DcaAddress){
        ...
        let resp = await instantiateContract(terra, wallet, wallet.key.accAddress, network.DcaCodeId, MSG, CONTRACT_LABEL)
